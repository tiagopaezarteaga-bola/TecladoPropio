...
class PersonalDictionary(private val context: Context) {
    ...
    fun learnWord(word: String, prevWord: String? = null) {
        val w = word.lowercase().trim().replace(PUNCT_REGEX, "")
    ...
    /**
     * Deshace un [learnWord] anterior: resta 1 a la frecuencia de la palabra, al
     * bigrama con [prevWord] y a los n-gramas de letras que ese aprendizaje sumó
     * (eliminando la entrada si llega a 0). Se usa cuando el usuario borra y
     * corrige una palabra recién completada, para que la edición no quede
     * registrada como si fuera una secuencia real de dos palabras distintas.
     * No toca swipePaths: conservar una trayectoria teórica de más es inofensivo.
     */
    fun unlearnWord(word: String, prevWord: String? = null) {
        val w = word.lowercase().trim().replace(PUNCT_REGEX, "")
        if (w.length < 2) return
        decrementWord(w)
        unrecordBigram(prevWord, w)
        unrecordLetterNgrams(w)
        save()
    }
    ...
    fun learnSwipeWord(
        ...
        // Guardar path de swipe solo si es coherente con el teórico (máx 10 muestras)
        val pathStr = pathChars.joinToString("").uppercase()
        val paths = swipePaths.getOrPut(w) {
            mutableListOf(computeTheoreticalSwipe(w))
        }
        val theoretical = paths.first()
        val dist = levenshtein(pathStr, theoretical)
        val normDist = dist.toFloat() / maxOf(pathStr.length, theoretical.length).coerceAtLeast(1)
        if (normDist <= SWIPE_PATH_THRESHOLD && paths.size < 10) {
            paths.add(pathStr)
        }
    ...
    /**
     * Sugiere las siguientes palabras más probables dado solo el contexto (sin prefix).
     * Usa exclusivamente bigramas: dado [prevWord], retorna las palabras que más
     * frecuentemente van después de ella.
     */
    fun suggestNextWord(prevWord: String, topN: Int = 3): List<String> {
        val prev = prevWord.lowercase().trim()
        if (prev.length < 2) return emptyList()
        val prefix = "${prev}_"
        return bigrams.entries
            .filter { (k, _) -> k.startsWith(prefix) }
            .map { (k, freq) -> k.removePrefix(prefix) to freq }
            .filter { (word, _) -> words.containsKey(word) }
            .sortedByDescending { it.second }
            .take(topN)
            .map { it.first }
    }
    ...
    fun importFrom(context: Context, uri: Uri): Int {
        ...
        try {
            ...
            if (count > 0) {
                // Para cada palabra importada sin trayectoria swipe, calcular la teórica.
                // Así el swipe funciona desde el primer uso sin que el usuario haya swipeado cada palabra.
                words.keys.forEach { word ->
                    if (!swipePaths.containsKey(word)) {
                        swipePaths[word] = mutableListOf(computeTheoreticalSwipe(word))
                    }
                }
                save()
            }
    ...
    /**
     * Elimina una palabra del diccionario junto con todos sus datos asociados
     * (swipe paths, gesture stats, penalizaciones y bigramas donde aparece).
     */
    fun removeWord(word: String) {
        val w = word.lowercase().trim()
        words.remove(w)
        swipePaths.remove(w)
        gestureStats.remove(w)
        swipePenalties.remove(w)
        // Eliminar bigramas donde aparece esta palabra (como primer o segundo término)
        val keysToRemove = bigrams.keys.filter { k ->
            k == "${w}_" || k.startsWith("${w}_") || k.endsWith("_${w}")
        }
        keysToRemove.forEach { bigrams.remove(it) }
        save()
    }
    ...
    private fun decrementWord(w: String) {
        val current = words[w] ?: return
        if (current <= 1) words.remove(w) else words[w] = current - 1
    }

    private fun recordBigram(prevWord: String?, word: String) {
        if (!prevWord.isNullOrBlank()) {
            val prev = prevWord.lowercase().trim().replace(PUNCT_REGEX, "")
            if (prev != word) {
                val key = "${prev}_${word}"
    ...
    private fun unrecordBigram(prevWord: String?, word: String) {
        if (!prevWord.isNullOrBlank()) {
            val prev = prevWord.lowercase().trim().replace(PUNCT_REGEX, "")
            if (prev != word) {
                val key = "${prev}_${word}"
                val current = bigrams[key] ?: return
                if (current <= 1) bigrams.remove(key) else bigrams[key] = current - 1
            }
        }
    }
    ...
    private fun unrecordLetterNgrams(word: String) {
        for (i in word.indices) {
            if (i + 1 < word.length) {
                val bg = word.substring(i, i + 2)
                val cur = letterNgrams[bg]
                if (cur != null) { if (cur <= 1) letterNgrams.remove(bg) else letterNgrams[bg] = cur - 1 }
            }
            if (i + 2 < word.length) {
                val tg = word.substring(i, i + 3)
                val cur = letterNgrams[tg]
                if (cur != null) { if (cur <= 1) letterNgrams.remove(tg) else letterNgrams[tg] = cur - 1 }
            }
        }
    }
    ...
    /** Signos de puntuación que se eliminan de palabras antes de guardarlas. */
    private val PUNCT_REGEX = Regex("[.,:;!?¡¿()\\[\\]{}'\"\\-…]")

    /**
     * Umbral de distancia Levenshtein normalizada para aceptar un path aprendido.
     * Un path cuya distancia al teórico supere este valor se descarta — probablemente
     * vino de una predicción incorrecta o de un gesto que no corresponde a la palabra.
     * Calibrado en 0.35: permite variaciones naturales del dedo sin aceptar rutas ajenas.
     */
    private val SWIPE_PATH_THRESHOLD = 0.35f
