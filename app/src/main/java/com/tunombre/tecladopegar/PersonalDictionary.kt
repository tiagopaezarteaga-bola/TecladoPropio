                ...
                BufferedReader(InputStreamReader(inp, Charsets.UTF_8)).forEachLine { line ->
                    ...
                    when {
                        ...
                        parts.size >= 5 && parts[0] == "GESTURE" -> {
                            val word = parts[1].lowercase().trim()
                            val len  = parts[2].removePrefix("avg_len=").toFloatOrNull() ?: continue
                            val spd  = parts[3].removePrefix("avg_spd=").toFloatOrNull() ?: continue
                            val n    = parts[4].removePrefix("n=").toIntOrNull() ?: continue
                            if (word.length >= 2) {
