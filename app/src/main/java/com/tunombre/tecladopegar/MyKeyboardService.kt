...
class MyKeyboardService : InputMethodService() {
    ...
    override fun onCreateInputView(): View {
        ...
        keyboardView.onCharTyped = { c ->
            ...
            // Si es signo de puntuación y hay un espacio justo antes, borrarlo
            // para que no quede "palabra ." sino "palabra."
            if (ic != null && isPunctuation(c)) {
                val before = ic.getTextBeforeCursor(1, 0)?.toString() ?: ""
                if (before == " ") ic.deleteSurroundingText(1, 0)
                // Aprender la palabra que precede al signo de puntuación
                learnCurrentWord()
            }
    ...
    private fun showTypingSuggestions(
        ...
        suggestions.forEachIndexed { i, word ->
            ...
            bar.addView(makeSuggestionView(display, bold = i == 0) {
     ...
     * actual de shift, sin consumirlo (solo para mostrar en la barra).
     */
    private fun capitalizeForDisplay(word: String): String {
        val kv = activeKeyboardView ?: return word
        return when {
            !kv.isShiftActive() -> word
            // Para saber si es caps lock: si applyShiftToWord pone todo en mayúscula
            // usamos una palabra de prueba de dos caracteres distintos
            kv.applyShiftToWord("az") == "AZ" -> word.uppercase()  // caps lock
            else -> word.replaceFirstChar { it.uppercaseChar() }    // single shift
        }
    }
    ...
    private fun makeSuggestionView(word: String, bold: Boolean, onClick: () -> Unit): TextView {
        return TextView(this).apply {
            ...
            setOnLongClickListener {
                val raw = word.lowercase().trim()
                personalDict.removeWord(raw)
                text = "✕ $word"
                setTextColor(Color.parseColor("#FF3B30"))
