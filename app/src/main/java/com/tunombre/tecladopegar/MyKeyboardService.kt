...
class MyKeyboardService : InputMethodService() {
    ...
    override fun onCreateInputView(): View {
        ...
        keyboardView.onBackspace = {
            val ic = currentInputConnection
            if (ic != null) {
                // Si había un swipe pendiente, el backspace significa que el usuario
                // rechaza la predicción: limpiar el estado del swipe para que no se
                // confirme accidentalmente en el siguiente disparador.
                if (pendingSwipeWord != null) {
                    pendingSwipeWord = null
                    lastSwipeCommittedWord = null
                    lastSwipePath = emptyList()
                }
                // El backspace solo borra. lastCompletedWord y el contexto de bigramas
                // NO se tocan: los bigramas se registran únicamente en los 4 disparadores
                // legítimos (espacio, puntuación, siguiente swipe, tap en sugerencia).
