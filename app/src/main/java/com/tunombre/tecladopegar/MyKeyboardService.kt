...
class MyKeyboardService : InputMethodService() {
    ...
    // (unlearnWord) y se restaura el contexto anterior, evitando bigramas espurios
    ...
    // Cuántos caracteres quedan por borrar de la palabra swipeada que se está corrigiendo.
    // Mientras sea > 0, cada backspace sigue perteneciendo a esa corrección y NO debe
    // actualizar lastCompletedWord (evita bigramas espurios con la palabra incorrecta).
    private var swipeCorrectionCharsLeft: Int = 0
    ...
    override fun onCreateInputView(): View {
        ...
        keyboardView.onLetterTyped = { letter ->
            ...
            // implica que el usuario siguió por su cuenta: ya no hay nada que confirmar.
        ...
        keyboardView.onBackspace = {
            val ic = currentInputConnection
            if (ic != null) {
                // ── Detección de corrección de swipe ──────────────────────────────
                // PRIMER backspace sobre la palabra swipeada: detectar, reportar error
                // y arrancar el contador de chars a borrar.
                if (lastSwipeCommittedWord != null) {
                    val w = lastSwipeCommittedWord!!
                    val expectedSuffix = if (pendingSwipeWord != null) w else "$w "
                    val before = ic.getTextBeforeCursor(w.length + 2, 0)?.toString() ?: ""
                    if (before.endsWith(expectedSuffix)) {
                        personalDict.reportSwipeError(w, lastSwipePath)
                        // Cuántos chars quedan DESPUÉS de este primer backspace.
                        // Si había espacio confirmado (pendingSwipeWord == null) el primer
                        // backspace borra el espacio → quedan w.length chars de la palabra.
                        // Si estaba pendiente (sin espacio) → quedan w.length - 1.
                        swipeCorrectionCharsLeft = if (pendingSwipeWord != null) w.length - 1 else w.length
                    }
                    lastSwipeCommittedWord = null
                    lastSwipePath = emptyList()
                    pendingSwipeWord = null
                    lastLearnedWord = null
                    lastLearnedWordPrev = null
                    lastCompletedWord = null
                }

                // BACKSPACES SIGUIENTES mientras aún quedan chars de la palabra incorrecta:
                // seguir sin tocar lastCompletedWord ni disparar unlearnWord.
                if (swipeCorrectionCharsLeft > 0) {
                    swipeCorrectionCharsLeft--
                    // No ejecutar el bloque de "palabra aprendida": el contexto ya fue
                    // limpiado al detectar la corrección y lastLearnedWord es null.
                }

                // ── Detección de corrección de palabra aprendida (escritura letra a letra) ──
                // Solo aplica cuando NO estamos en medio de borrar una corrección de swipe.
                if (swipeCorrectionCharsLeft == 0) {
                    val lw = lastLearnedWord
                    if (lw != null) {
                        val probe = ic.getTextBeforeCursor(lw.length + 1, 0)?.toString() ?: ""
                        if (probe.length == lw.length + 1 && probe.dropLast(1).equals(lw, ignoreCase = true)) {
                            personalDict.unlearnWord(lw, lastLearnedWordPrev)
                            lastCompletedWord = lastLearnedWordPrev
                            lastLearnedWord = null
                            lastLearnedWordPrev = null
                        }
                    }
        ...
        keyboardView.onSpace = {
            // El propio espacio manual confirma cualquier swipe pendiente.
            pendingSwipeWord = null
            learnCurrentWord()
        ...
        keyboardView.onSwipeSuggestions = { _, pathChars, totalLenPx, durationMs, pauseIndices ->
            ...
            // Si ya había una palabra swipeada pendiente de confirmar (sin espacio aún),
            // este nuevo swipe la confirma implícitamente: ahora sí se agrega el espacio
            // que quedó pendiente ("el espacio se agrega en el siguiente swipeo").
    ...
    private fun confirmSwipeSelection(word: String, isAlternative: Boolean) {
        ...
        // Confirmar: agregar ahora el espacio que había quedado pendiente.
        ic.commitText(" ", 1)
        pendingSwipeWord = null
        lastSwipeCommittedWord = null
        lastSwipePath = emptyList()
        // La confirmación es también el disparador del ciclo BIGRAM.
        updateTypingSuggestions()
    }
