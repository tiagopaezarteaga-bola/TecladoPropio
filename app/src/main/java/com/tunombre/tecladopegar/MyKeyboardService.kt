...
class MyKeyboardService : InputMethodService() {
    ...
    // Letras efectivamente tecleadas en la palabra actual (letra a letra).
    // Solo se aprende si la palabra leída del campo coincide con lo que se tecleó aquí.
    // Evita aprender palabras que ya estaban en el campo antes de que el usuario escribiera.
    private val currentTypedWord = StringBuilder()
    ...
    override fun onCreateInputView(): View {
        ...
        keyboardView.onBackspace = {
            ...
            if (ic != null) {
                if (pendingSwipeWord != null) {
                    // El backspace sobre una palabra swipeada pendiente cancela el estado de swipe
                    // pero borra solo un carácter (comportamiento normal de borrado letra a letra).
                    pendingSwipeWord = null
                    lastSwipeCommittedWord = null
                    lastSwipePath = emptyList()
                    ...
                    if (currentTypedWord.isNotEmpty()) currentTypedWord.deleteCharAt(currentTypedWord.length - 1)
                    val selected = ic.getSelectedText(0)
                    if (!selected.isNullOrEmpty()) {
                        ic.commitText("", 0)
                    } else {
    ...
    private fun learnCurrentWord() {
        ...
        // Solo aprender si la palabra leída del campo coincide (ignorando mayúsculas)
        // con lo que el usuario efectivamente tecleó en esta sesión. Esto evita aprender
        // texto que ya estaba en el campo antes de que el usuario empezara a escribir
        // (por ejemplo, al poner el cursor en medio de un texto existente y pulsar espacio).
        if (word.length >= 2 && word.equals(currentTypedWord.toString(), ignoreCase = true)) {
            val prevForLearn = lastCompletedWord
            personalDict.learnWord(word, prevForLearn)
            lastCompletedWord = word
            lastLearnedWord = word
