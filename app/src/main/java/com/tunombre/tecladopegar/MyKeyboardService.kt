    ...
    override fun onCreateInputView(): View {
        ...
        keyboardView.onBackspace = {
            ...
            if (ic != null) {
                ...
                var swipeCorrectionDetected = false
                if (lastSwipeCommittedWord != null) {
                    val w = lastSwipeCommittedWord!!
                    // Si aún está pendiente de confirmar, no hay espacio tras la palabra;
                    // si ya fue confirmada, sí lo hay ("palabra ").
                    ...
                    if (before.endsWith(expectedSuffix)) {
                        ...
                        swipeCorrectionDetected = true
                    }
                    lastSwipeCommittedWord = null
                    lastSwipePath = emptyList()
                    pendingSwipeWord = null
                    // Limpiar también el contexto de "palabra aprendida" para que el bloque
                    // siguiente no lo confunda con una corrección de escritura letra a letra.
                    lastLearnedWord = null
                    lastLearnedWordPrev = null
                    // Resetear lastCompletedWord para que el próximo learnWord no registre
                    // un bigrama con la palabra incorrecta del swipe fallido.
                    lastCompletedWord = null
                }

                // Detección de corrección de palabra recién aprendida: si el usuario está
                // borrando justo el separador (espacio, punto, coma, salto de línea, etc.)
                // que sigue a la última palabra ya sumada al diccionario, es señal de que
                // va a reescribirla. Deshacemos ese aprendizaje (word+bigrama+n-gramas) y
                // restauramos el contexto anterior, para no dejar un bigrama espurio como
                // "comienza_comience" cuando en realidad es una sola palabra corregida.
                // GUARD: si ya procesamos una corrección de swipe en este backspace,
                // no ejecutar este bloque: el estado ya fue limpiado arriba.
                if (!swipeCorrectionDetected) {
