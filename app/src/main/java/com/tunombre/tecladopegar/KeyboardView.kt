...
sealed class KeyType {
    ...
    object BackToLetters : KeyType()   // Números/Símbolos -> Letras (ABC)
    ...
    object CommaEmoji : KeyType()      // toque = coma, mantener presionado = emoji
        ...
        keys.forEach { key ->
            ...
            val paint = when {
                ...
                key.type is KeyType.Shift || key.type is KeyType.Backspace ||
                    key.type is KeyType.Numbers123 || key.type is KeyType.BackToLetters ||
                    key.type is KeyType.ModeSwitchLeft || key.type is KeyType.NumSymToggle -> specialKeyPaint
                else -> keyPaint
            }
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (key?.type is KeyType.CommaEmoji) {
                    ...
                    longPressHandler.postDelayed(runnable, 350L)
                }
            }
