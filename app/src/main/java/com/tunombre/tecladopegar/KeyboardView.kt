...
class KeyboardView(context: Context) : ViewGroup(context) {
    ...
    fun isShiftActive(): Boolean = keysCanvas.isShiftActive()

    fun isCapsLockActive(): Boolean = keysCanvas.isCapsLockActive()
...
class KeysCanvasView(context: Context) : View(context) {
     ...
     */
    /**
     * Indica si el shift está activo (SINGLE o CAPS_LOCK) sin modificarlo.
     * Útil para decidir el display visual de sugerencias.
     */
    fun isShiftActive(): Boolean = shiftState != ShiftState.OFF

    fun isCapsLockActive(): Boolean = shiftState == ShiftState.CAPS_LOCK
