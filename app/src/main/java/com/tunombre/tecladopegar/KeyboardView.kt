...
class KeysCanvasView(context: Context) : View(context) {
    ...
    private fun drawKeyContent(canvas: Canvas, key: KeyInfo) {
        ...
        when (val type = key.type) {
            ...
            KeyType.Shift -> {
                when (shiftState) {
                    ShiftState.OFF -> {
                        // Contorno hueco: dibujamos el glifo con color transparente y stroke
                        ...
                        canvas.drawText("⇧", cx, cy + 8f, outlinePaint)
                    }
                    ShiftState.SINGLE -> {
                        // Flecha rellena negra normal
                        canvas.drawText("⇧", cx, cy + 8f, textPaint)
                    }
                    ShiftState.CAPS_LOCK -> {
                        // Flecha rellena blanca + barra baja blanca
                        canvas.drawText("⇧", cx, cy + 8f, whiteTextPaint)
    ...
    private fun showAccentPopup(key: KeyInfo, variants: List<String>) {
        ...
        val popupY = key.rect.top - cellH - 8f
        // Centrar el popup sobre la tecla, ajustando para no salir de la pantalla
